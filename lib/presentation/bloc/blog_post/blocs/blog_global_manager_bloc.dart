...
class BlogPostGlobalManagerBloc extends Bloc<BlogPostEvent, BlogPostState> {
  BlogPostGlobalManagerBloc() : super(BlogPostInitial()) {
    on<FetchAllBlogPosts>(_onFetchAllBlogPosts);
    on<SearchBlogPosts>(_onSearchBlogPosts);
  }
  ...
  void _onSearchBlogPosts(
      SearchBlogPosts event, Emitter<BlogPostState> emit) async {
    emit(BlogPostInitial()); // Affiche le CircularProgressIndicator
    ...
    try {
      ...
      final List<BlogPost> blogPosts = await BlogPostUsecase(
          ...
          .getSearchBlogPost(event.searchQuery);
